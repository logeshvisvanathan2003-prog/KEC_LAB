...
class KCELoginApp:
    ...
    def _entry(self, parent, show=None):
        ...
        def on_focus_in(ev, _e=e, _f=f):
            _f.config(highlightbackground=ACCENT, highlightthickness=2)
            if _e.cget('fg') == '#9ca3af':
                _e.delete(0, 'end')
                _e.config(fg=DARK, show=_e._real_show if hasattr(_e,'_real_show') else '')
        def on_focus_out(ev, _e=e, _f=f):
            _f.config(highlightbackground=BORDER, highlightthickness=1)
    ...
    def _build(self):
        ...
        # Email placeholder
        self.email_entry.insert(0, 'your@kongu.edu')
        self.email_entry.config(fg='#9ca3af')
        def email_focus_in(e):
            if self.email_entry.get() == 'your@kongu.edu':
                self.email_entry.delete(0, 'end')
                self.email_entry.config(fg=DARK)
        def email_focus_out(e):
        ...
        self.pwd_entry.insert(0, 'Enter your password')
        self.pwd_entry.config(fg='#9ca3af')
        def pwd_focus_in(e):
            if self.pwd_entry.get() == 'Enter your password':
                self.pwd_entry.delete(0, 'end')
                self.pwd_entry.config(fg=DARK, show='•')
        def pwd_focus_out(e):
            if not self.pwd_entry.get().strip():
                self.pwd_entry.config(show='')
                self.pwd_entry.insert(0, 'Enter your password')
                self.pwd_entry.config(fg='#9ca3af')
        self.pwd_entry.bind('<FocusIn>', pwd_focus_in)
        self.pwd_entry.bind('<FocusOut>', pwd_focus_out)
        ...
        # Secret admin shortcut: Ctrl+Alt+S to skip
        self.root.bind('<Control-Alt-s>', lambda e: self._admin_skip())
        self.root.bind('<Control-Alt-S>', lambda e: self._admin_skip())
    ...
    def _login(self):
        ...
        # ignore placeholders
        if email == 'your@kongu.edu': email = ''
        if pwd == 'Enter your password': pwd = ''
    ...
    def _admin_skip(self):
        # Secret shortcut for admin/lab tech only
        self.root.grab_release()
        self.root.destroy()
